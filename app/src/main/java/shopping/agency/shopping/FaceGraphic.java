...
import shopping.agency.shopping.ui.camera.GraphicOverlay;
...
private  int imagedrawable;
  ...
  FaceGraphic(GraphicOverlay overlay, Context context, boolean isFrontFacing,int imagedrawable) {
    super(overlay);
    mIsFrontFacing = isFrontFacing;
    Resources resources = context.getResources();
    initializePaints(resources);
    this.imagedrawable=imagedrawable;
    System.out.println("imagedrawable arsh= " + imagedrawable);
  ...
  private void initializeGraphics(Resources resources) {
    ...
    mHappyStarGraphic = resources.getDrawable(R.drawable.sunglass1);
    try {
      if(this.imagedrawable==123) {
        mHatGraphic = resources.getDrawable(R.drawable.hat);
      }
    else   if(this.imagedrawable==122) {
        mHatGraphic = resources.getDrawable(R.drawable.cap1);
      }
  else {
        mHappyStarGraphic = resources.getDrawable(R.drawable.sunglass1);
      }
  ...
  public void draw(Canvas canvas) {
   ...
   // drawMustache(canvas, noseBasePosition, mouthLeftPosition, mouthRightPosition);
    ...
    //if (Math.abs(eulerZ) > HEAD_TILT_HAT_THRESHOLD) {
             if(this.imagedrawable==121) {
               drawEye1(canvas, leftEyePosition, eyeRadius, leftIrisPosition, irisRadius, leftEyeOpen, smiling,width);
  ...
  private void drawEye1(Canvas canvas,
                       PointF eyePosition, float eyeRadius,
                       PointF irisPosition, float irisRadius,
                       boolean eyeOpen, boolean smiling,float faceWidth) {
    float eyeareawidth = faceWidth-eyeRadius;

    mHappyStarGraphic.setBounds(
              (int) (eyePosition.x-eyeRadius -eyeareawidth/2),
              (int) (eyePosition.y - irisRadius),
              (int) (eyePosition.x + eyeareawidth/3),
              (int) (eyePosition.y + irisRadius));
      mHappyStarGraphic.draw(canvas);
  ...
  private void drawHat(Canvas canvas, PointF facePosition, float faceWidth, float faceHeight, PointF noseBasePosition) {
   ...
   // final float HAT_CENTER_Y_OFFSET_FACTOR = (float)(1.0 / 8.0);
    ...
    int top = (int)(hatCenterY - (hatHeight*1.4 ));
    int bottom = (int)(hatCenterY + (hatHeight / 3));
